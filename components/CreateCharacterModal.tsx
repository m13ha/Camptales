...
export const CreateCharacterModal: React.FC<CreateCharacterModalProps> = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleGeneratePreview = async () => {
    ...
    try {
      ...
      alert(`Could not generate portrait: ${errorMessage}`);
    } finally {
  ...
  const handleSaveCharacter = (e: React.FormEvent) => {
    ...
    if (!newChar.name.trim() || !newChar.description.trim() || !newChar.imageUrl) {
        alert("Please make sure you have a description, a generated portrait, and a name for your character.");
        return;
    };
